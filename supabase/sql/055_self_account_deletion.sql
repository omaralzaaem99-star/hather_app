-- 055: Self-service account deletion (prepare + preserve order history)
-- Depends on: profiles, delivery_orders, push_device_tokens, admin_user_deletion_audit
-- Does NOT reset request_number. Does NOT touch app_settings / coupons definitions.

-- ---------------------------------------------------------------------------
-- 1) Preserve historical orders when a profile is deleted
--    Was: user_id NOT NULL … ON DELETE CASCADE (wipes all orders)
--    Now: user_id NULLABLE … ON DELETE SET NULL + PII anonymized before Auth delete
-- ---------------------------------------------------------------------------
...
comment on column public.delivery_orders.user_id is
  'Customer profile. NULL after account deletion (order history retained, PII stripped).';

-- ---------------------------------------------------------------------------
-- 2) prepare_my_account_deletion — caller = auth.uid() only
-- ---------------------------------------------------------------------------
...
begin
  ...
  if prof.id is null then
    -- Idempotent: profile already gone
    return jsonb_build_object('ok', true, 'already_deleted', true);
  end if;

  -- Active as customer (pending = searching / waiting; active = in delivery)
  ...
  -- Anonymize retained order history (terminal statuses only)
  ...
  -- Clear captain assignment on non-active retained rows (belt & suspenders)
  update public.delivery_orders
  set
    captain_id = null,
    updated_at = now()
  ...
  -- Device tokens (also cascade from auth.users)
  ...
  return jsonb_build_object(
    ...
    'had_active_subscription_warning', false
  );
end;
$$;
...
comment on function public.prepare_my_account_deletion() is
  'Prepares self-service account deletion for auth.uid(): blocks active orders, anonymizes history, clears FCM tokens. Auth user delete is Edge-only.';
